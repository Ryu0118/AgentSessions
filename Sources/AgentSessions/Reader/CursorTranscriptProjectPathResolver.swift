...
public struct CursorTranscriptProjectPathResolver: Sendable {
    ...
    public func resolveProjectPath(for transcriptFile: URL) -> String? {
        let workspacePath = decodeProjectPath(fromTranscriptFile: transcriptFile)

        if let headData = try? FileSystemHelper.readHead(fileSystem: fileSystem, file: transcriptFile, maxBytes: 131_072),
           let path = projectPath(inTranscriptData: headData, workspacePath: workspacePath)
        {
            return path
        }

        if let tailData = try? FileSystemHelper.readTail(fileSystem: fileSystem, file: transcriptFile, maxBytes: 262_144),

...
public enum CursorAgentTag: String, CaseIterable, Sendable {
    case userQuery = "user_query"

    public var openingTag: String { "<\(rawValue)>" }
    public var closingTag: String { "</\(rawValue)>" }

    public func wrap(_ content: String) -> String {
        "\(openingTag)\n\(content)\n\(closingTag)"
    }
}
...
public enum CursorAgentContentDecoder: Sendable {
